...
export default async function AddRoomPage() {
  const floors = await fetchData("main/floors/");
  const roomTypes = await fetchData("main/room-types/");
  ...
  async function createRoom(formData: FormData) {
    ...
    const response = await postData("main/rooms/", room);
    ...
    <div className="flex items-center justify-center">
      <form className="space-y-2 p-2 w-full lg:w-2/3" action={createRoom}>
        ...
        <div className="flex flex-row justify-between">
          ...
          <button type="submit" className="btn rounded-sm btn-primary mt-3">
            Create
          </button>
        </div>
      </form>
