...
export default async function Page() {
    const data = await Fetch({ "endpoint": "iot/device-types/" });
                    ...
                    <tbody>
                        {deviceTypes.map((deviceType: deviceType) => (
                            <tr key={deviceType.id}>
                                ...
                                <td>
                                    {deviceType.description ? deviceType.description : 'No description'}
                                </td>
                                <td>
                                    <Link href={`/dashboard/devices/type/${deviceType.id}`} className="hover:underline hover:text-blue-700">
                                        <FileSymlink size={20} />
                                    </Link>
                                </td>
                            </tr>
